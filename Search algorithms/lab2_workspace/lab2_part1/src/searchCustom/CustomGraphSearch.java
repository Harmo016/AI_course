...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		for (GridPos child : childStates) {
			SearchNode childNode = new SearchNode(child, new SearchNode(startState));
		...
		while(startLoop) {
			System.out.println("Caught in looop ");
			SearchNode searchingNode = frontier.peekAtFront();
			GridPos searching = searchingNode.getState();
			System.out.println(searching);
			if(p.isGoalState(searching)) {
				startLoop = false; 
				path = searchingNode.getPathFromRoot();
			}
				...
				childStates = p.getReachableStatesFrom(searching);
				for (GridPos child : childStates) {
					SearchNode childNode = new SearchNode(child, searchingNode);
